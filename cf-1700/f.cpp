...
int main(int, char const *[]) {
	...
	RF(i, 0, N) {
		...
		if (s[i].first + s[i].second == t[i].first + t[i].second) {
			ans++;
			s[i] = t[i];
			continue;
		}
		while (s[i].first + s[i].second < t[i].first + t[i].second) {
			while (s[n1] == PR<LL, LL>{0, 0}) {
				n1++;
			}
			ans += n1 - i;
			if (t[i].first > s[i].first) {
				if (s[n1].first) {
					swap(s[i].first, s[n1].first);
				} else {
					swap(s[i].first, s[n1].second);
					ans++;
				}
				continue;
			}
			if (t[i].second > s[i].second) {
				if (s[n1].second) {
					swap(s[i].second, s[n1].second);
				} else {
					swap(s[i].second, s[n1].first);
					ans++;
				}
			}
		}
		while (s[i].first + s[i].second > t[i].first + t[i].second) {
			while (s[n0] == PR<LL, LL>{1, 1}) {
				n0++;
			}
			ans += n0 - i;
			if (t[i].first < s[i].first) {
				if (!s[n0].first) {
					swap(s[i].first, s[n0].first);
				} else {
					swap(s[i].first, s[n0].second);
					ans++;
				}
				continue;
			}
			if (t[i].second < s[i].second) {
				if (!s[n0].second) {
					swap(s[i].second, s[n0].second);
				} else {
					swap(s[i].second, s[n0].first);
					ans++;
				}
			}
		}
