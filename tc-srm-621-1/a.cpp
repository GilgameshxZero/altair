...
class RadioRange
{
	public:
		...
		{
			queue< pair<double, double> > range;

			range.push (make_pair (0, double (Z)));

			for (int a = 0;a < X.size ();a++)
			{
				double first, last, dist = sqrt ((double)X[a] * X[a] + (double)Y[a] * Y[a]);

				first = dist - (double)R[a];
				last = dist + (double)R[a];

				//remove interval
				int b = range.size ();
				for (int c = 0;c < b;c++)
				{
					pair<double, double> curr = range.front ();
					range.pop ();

					if (last < curr.first || first > curr.second)
					{
						range.push (curr);
						continue;
					}
					if (first > curr.first)
						range.push (make_pair (curr.first, first));
					if (last < curr.second)
						range.push (make_pair (last, curr.second));
				}
			}
