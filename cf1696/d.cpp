...
int main(int, char const *[]) {
	...
	while (T--) {
		...
		LL lgN{(LL)mostSignificant1BitIdx(N) + 1};
		VR<VR<LL>> rmin(lgN), rmax(lgN);
		rmin[0].resize(N);
		rmax[0].resize(N);
		RF(i, 0, N) {
		...
		RF(i, 1, lgN) {
			...
			RF(j, 0, N) {
				rmin[i][j] =
					min(rmin[i - 1][j], rmin[i - 1][min(N - 1, j + (1LL << (i - 1)))]);
				rmax[i][j] =
					max(rmax[i - 1][j], rmax[i - 1][min(N - 1, j + (1LL << (i - 1)))]);
			}
		}
		VR<LL> B(N);
		B[N - 1] = N - 1;
		...
		RF(i, N - 3, -1) {
			if (A[i] > A[i + 1]) {
				...
				while (high - 1 != low) {
					mid = (low + high) / 2;
					LL lg{(LL)mostSignificant1BitIdx(mid + 1 - i)};
					if (max(rmax[lg][i], rmax[lg][mid + 1 - (1LL << lg)]) > A[i]) {
						high = mid;
					} else {
						low = mid;
					}
				...
				LL border{high};
				LL rngLg{(LL)mostSignificant1BitIdx(border - i)};
				LL argMin{min(rmin[rngLg][i], rmin[rngLg][border - (1LL << rngLg)])};
				LL posMin{pos[argMin]};
				B[i] = posMin;
			} else {
				LL low{i}, high{N}, mid;
				while (high - 1 != low) {
					mid = (low + high) / 2;
					LL lg{(LL)mostSignificant1BitIdx(mid + 1 - i)};
					if (min(rmin[lg][i], rmin[lg][mid + 1 - (1LL << lg)]) < A[i]) {
						high = mid;
					} else {
						low = mid;
					}
				...
				LL border{high};
				LL rngLg{(LL)mostSignificant1BitIdx(border - i)};
				LL argMax{max(rmax[rngLg][i], rmax[rngLg][border - (1LL << rngLg)])};
				LL posMax{pos[argMax]};
				B[i] = posMax;
			}
		}

		VR<LL> dist(N, 1000000000);
		dist.back() = 0;
		VR<VR<LL>> rB(N);
		RF(i, 0, N) {
			rB[B[i]].push_back(i);
		...
		RF(i, N - 1, 0) {
			RF(j, 0, rB[i].size()) {
				dist[rB[i][j]] = min(dist[rB[i][j]], dist[i] + 1);
			}
		}
		cout << dist[0] << '\n';
	}
