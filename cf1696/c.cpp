...
int main(int, char const *[]) {
	...
	while (T--) {
		...
		RF(i, 0, N) {
			cA.push_back({A[i], 1});
			while (cA.back().first % M == 0) {
				cA.back().first /= M;
				cA.back().second *= M;
			}
			if (cA.size() >= 2 && cA.back().first == cA[cA.size() - 2].first) {
				cA[cA.size() - 2].second += cA.back().second;
				cA.pop_back();
			}
		}
		RF(i, 0, K) {
			cB.push_back({B[i], 1});
			while (cB.back().first % M == 0) {
				cB.back().first /= M;
				cB.back().second *= M;
			}
			if (cB.size() >= 2 && cB.back().first == cB[cB.size() - 2].first) {
				cB[cB.size() - 2].second += cB.back().second;
				cB.pop_back();
			}
		}
		cout << (cA == cB ? "Yes\n" : "No\n");
