...
int main(int, char const *[]) {
	...
	if (AX + R <= BX || AY + C <= BY) {
		cout << MR::power(K, N * M - R * C);
		return 0;
	}

	pair<LL, LL> delta{BX - AX, BY - AY},
		overlap{R - delta.first, C - delta.second};
	if (delta.first == 0 && delta.second == 0) {
		cout << MR::power(K, N * M);
		return 0;
	}
	if (overlap.first * 2 <= R || overlap.second * 2 <= C) {
		LL overlappers{overlap.first * overlap.second},
			frees{R * C - overlappers * 2};
		cout << MR::power(
			K,
			N * M - R * C * 2 + overlap.first * overlap.second + overlappers +
				frees);
		return 0;
	}

	LL frees{2 * delta.first * delta.second},
		overlappers{
			delta.first * overlap.second + delta.second * overlap.first -
			delta.first * delta.second};
	cout << MR::power(
		K,
		N * M - R * C * 2 + overlap.first * overlap.second + overlappers + frees);
	return 0;
}
