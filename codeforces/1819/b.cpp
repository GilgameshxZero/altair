...
int main(int, char const *[]) {
	...
	while (T--) {
		...
		if (N == 1) {
			Z.insert({HW.begin()->first, HW.begin()->second});
		} else {
			if (HW.begin()->first == HW.rbegin()->first) {
				LL TW{0};
				for (auto &i : HW) {
					TW += i.second;
				}
				Z.insert({HW.begin()->first, TW});
			}
			if (WH.begin()->first == WH.rbegin()->first) {
				LL TH{0};
				for (auto &i : WH) {
					TH += i.second;
				}
				Z.insert({TH, WH.begin()->first});
			}
			{
				auto HWc{HW}, WHc{WH};
				pair<LL, LL> C{0, 0}, R;
				C.first = R.first = HW.rbegin()->first;
				while (!HW.empty() && HW.rbegin()->first == R.first) {
					C.second += HW.rbegin()->second;
					WH.erase(WH.find({HW.rbegin()->second, HW.rbegin()->first}));
					HW.erase(HW.find(*HW.rbegin()));
				}
				if (!WH.empty()) {
					C.second += WH.rbegin()->first;
					R.second = WH.rbegin()->first;
					while (!WH.empty() && WH.rbegin()->first == R.second) {
						R.first -= WH.rbegin()->second;
						HW.erase(HW.find({WH.rbegin()->second, WH.rbegin()->first}));
						WH.erase(WH.find(*WH.rbegin()));
					}
					bool vert{true}, possible{true};
					while (!HW.empty() && !WH.empty()) {
						if (vert) {
							LL target{HW.rbegin()->first};
							if (target != R.first) {
								possible = false;
								break;
							}
							while (!HW.empty() && HW.rbegin()->first == target) {
								R.second -= HW.rbegin()->second;
								WH.erase(WH.find({HW.rbegin()->second, HW.rbegin()->first}));
								HW.erase(HW.find(*HW.rbegin()));
							}
						} else {
							LL target{WH.rbegin()->first};
							if (target != R.second) {
								possible = false;
								break;
							}
							while (!WH.empty() && WH.rbegin()->first == target) {
								R.first -= WH.rbegin()->second;
								HW.erase(HW.find({WH.rbegin()->second, WH.rbegin()->first}));
								WH.erase(WH.find(*WH.rbegin()));
							}
					...
					if (possible && R.first * R.second == 0) {
						Z.insert(C);
					}
				}
				HW.swap(HWc);
			...
			{
				pair<LL, LL> C{0, 0}, R;
				C.second = R.second = WH.rbegin()->first;
				while (!WH.empty() && WH.rbegin()->first == R.second) {
					C.first += WH.rbegin()->second;
					HW.erase(HW.find({WH.rbegin()->second, WH.rbegin()->first}));
					WH.erase(WH.find(*WH.rbegin()));
				}
				if (!WH.empty()) {
					C.first += HW.rbegin()->first;
					R.first = HW.rbegin()->first;
					while (!HW.empty() && HW.rbegin()->first == R.first) {
						R.second -= HW.rbegin()->second;
						WH.erase(WH.find({HW.rbegin()->second, HW.rbegin()->first}));
						HW.erase(HW.find(*HW.rbegin()));
					}
					bool vert{false}, possible{true};
					while (!HW.empty() && !WH.empty()) {
						if (vert) {
							LL target{HW.rbegin()->first};
							if (target != R.first) {
