...
bool ff(VR<set<PR<LL, bool>>> &edIr, VR<LL> &noDir, LL cur) {
	for (auto i : edIr[cur]) {
		if (noDir[i.first] == 0) {
			noDir[i.first] = noDir[cur] * -1;
			ff(edIr, noDir, i.first);
		} else if (noDir[i.first] == noDir[cur] * -1) {
		} else {
			return true;
		}
	}
...
int main(int, char const *[]) {
	...
	VR<set<PR<LL, bool>>> edIr(N);
	RF(i, 0, M) {
		LL j, k, l;
		cin >> j >> k >> l;
		edIr[k - 1].insert({l - 1, j == 1});
		edIr[l - 1].insert({k - 1, j == 1});
	}
	...
	RF(i, 0, N) {
		for (auto j : edIr[i]) {
			outDeg[i] += noDir[i] == 1 && j.second || noDir[i] == -1 && !j.second;
		}
		if (outDeg[i] == 0) {
			q.push(i);
		}
	...
	while (!q.empty()) {
		...
		q.pop();
		for (auto i : edIr[order.back()]) {
			if (
				noDir[i.first] == 1 && i.second || noDir[i.first] == -1 && !i.second) {
				outDeg[i.first]--;
				if (outDeg[i.first] == 0) {
					q.push(i.first);
				}
	...
	VR<LL> loc(N);
	RF(i, 0, order.size()) { loc[order[i]] = i; }
	cout << "YES\n";
	RF(i, 0, loc.size()) {
		cout << (noDir[i] == 1 ? 'R' : 'L') << ' ' << loc[i] << '\n';
	}
