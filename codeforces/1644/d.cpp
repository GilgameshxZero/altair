...
int main(int, char const *[]) {
	...
	while (T--) {
		LL N, M, K, Q;
		cin >> N >> M >> K >> Q;
		unordered_set<LL> rows, cols, valids;
		VR<PR<LL, LL>> ops(Q);
		RF(i, 0, Q) { cin >> ops[i].first >> ops[i].second; }
		reverse(ops.begin(), ops.end());
		RF(i, 0, Q) {
			if (rows.find(ops[i].first) == rows.end() && cols.size() < M) {
				rows.insert(ops[i].first);
				valids.insert(i);
			}
			if (cols.find(ops[i].second) == cols.end() && rows.size() < N) {
				cols.insert(ops[i].second);
				valids.insert(i);
			}
		}
		cout << ModRing<LL, 998244353>::power({K}, valids.size()).value << '\n';
	}
