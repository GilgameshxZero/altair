...
class AddMinPolicy {
	...
	using Result = pair<LL, LL>;

	static constexpr Value DEFAULT_VALUE{LLONG_MAX / 2, 0};
	static constexpr Update DEFAULT_UPDATE{0, 0};

	static void apply(Value &value, Update const &update, std::size_t range) {
		value.first += update.first;
		value.second += update.second;
	}
	...
	split(Update const &update, Update &left, Update &right, std::size_t range) {
		left.first += update.first;
		left.second += update.second;
		right.first += update.first;
		right.second += update.second;
	}
};
...
int main() {
	...
	while (T--) {
		LL N, K;
		cin >> N >> K;
		// SegmentTreeLazy<AddMinPolicy> T(N);
		vector<LL> A(N);
		vector<bool> Y(N);
		multimap<LL, LL> S;
		RF(i, 0, N) {
			cin >> A[i];
			S.insert({-A[i] + N - 1 - i, i});
			// T.update(i, i, {-LLONG_MAX / 2 - A[i] + N - 1 - i, i});
		}
		RF(i, 0, K) {
			auto q{S.begin()};
			Y[q->second] = true;
			// T.update(0, q.second - 1, {-1, 0});
			// T.update(q.second, q.second, {LLONG_MAX / 2, 0});
			S.erase(q);
		}
