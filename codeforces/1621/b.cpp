...
int main(int argc, char const *argv[]) {
	...
	while (T--) {
		...
		VR<PR<LL, LL>> seg(N);
		VR<LL> cost(N);
		RF(i, 0, N) { cin >> seg[i].first >> seg[i].second >> cost[i]; }

		map<PR<LL, LL>, LL> avail;
		PR<LL, LL> cheap, bound = seg[0];

		avail.insert({seg[0], cost[0]});
		cout << cost[0] << '\n';
		RF(i, 1, N) {
			bound = {
				min(bound.first, seg[i].first), max(bound.second, seg[i].second)};

			if (avail.find(seg[i]) == avail.end()) {
				avail[seg[i]] = cost[i];
			} else {
				avail[seg[i]] = min(cost[i], avail[seg[i]]);
			}

			if (seg[i].first < seg[cheap.first].first) {
				cheap.first = i;
			} else if (seg[i].first == seg[cheap.first].first) {
				if (cost[i] < cost[cheap.first]) {
					cheap.first = i;
				}
			}
			if (seg[i].second > seg[cheap.second].second) {
				cheap.second = i;
			} else if (seg[i].second == seg[cheap.second].second) {
				if (cost[i] < cost[cheap.second]) {
					cheap.second = i;
				}
			}

			if (avail.find(bound) == avail.end()) {
				cout << cost[cheap.first] + cost[cheap.second] << '\n';
			} else {
				if (avail[bound] < cost[cheap.first] + cost[cheap.second]) {
					cout << avail[bound] << '\n';
				} else {
					cout << cost[cheap.first] + cost[cheap.second] << '\n';
				}
