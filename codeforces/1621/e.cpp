	...
	while (T--) {
		...
		RF(i, 0, M) {
			...
			RF(j, 0, K) {
				LL tmp;
				cin >> tmp;
				groups.back().second.push_back(tmp);
				groups.back().first += tmp;
			}
			groups.back().first /= groups.back().second.size();
		}
		sort(teachers.begin(), teachers.end());
		sort(groups.begin(), groups.end());
		reverse(teachers.begin(), teachers.end());
		reverse(groups.begin(), groups.end());
		while (teachers.size() > groups.size()) {
			teachers.pop_back();
		}

		VR<bool> can(M, false), can2(M, false);
		can[0] = teachers[0] >= groups[0].first;
		RF(i, 1, M) {
			if (
				teachers[i] >= groups[i].first && teachers[i] >= groups[i - 1].first) {
				can[i] = true;
				break;
			}
		}
		VR<LL> pre(M);
		pre[0] = can[0];
		RF(i, 1, M) { pre[i] = can[i] + pre[i - 1]; }

		RF(i, 0, M) {
			RF(j, 0, groups[i].second.size()) {
					LD next =
						(LD)(groups[i].first * groups[i].second.size() - groups[i].second[j]) /
						(groups[i].second.size() - 1);
					auto idx =
						lower_bound(
							groups.begin(),
							groups.end(),
							PR<LD, VR<LL>>{next, {}},
							[](PR<LD, VR<LL>> const &left, PR<LD, VR<LL>> const &right) {
								return left.first > right.first;
							}) -
						groups.begin();
					if (pre[i] - pre[idx] == 0 && teachers[idx] >= next) {
						cout << '1';
					} else {
						cout << '0';
					}
