...
int main(int, char const *[]) {
	...
	LL N, M;
	cin >> N >> M;
	array<vector<LL>, 18> rmaxq;
	rmaxq[0].resize(M);
	RF(i, 0, M) {
		cin >> rmaxq[0][i];
	}
	RF(i, 1, rmaxq.size()) {
		rmaxq[i].resize(max(0LL, M + 1 - (1LL << i)));
		RF(j, 0, rmaxq[i].size()) {
			rmaxq[i][j] = max(rmaxq[i - 1][j], rmaxq[i - 1][j + (1LL << (i - 1))]);
		}
	}
	LL Q;
	cin >> Q;
	RF(i, 0, Q) {
		...
		LL top{X1 + (N - X1) / K * K},
			ring{(LL)mostSignificant1BitIdx(Y2 - Y1 + 1)},
			high{max(rmaxq[ring][Y1 - 1], rmaxq[ring][Y2 - (1LL << ring)])};
		cout << (high < top ? "YES" : "NO") << '\n';
	}
