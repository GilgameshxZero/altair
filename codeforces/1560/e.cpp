...
int main(int argc, char const *argv[]) {
...
#ifndef ONLINE_JUDGE
	std::freopen("e-in.txt", "r", stdin);
	std::freopen("e-out.txt", "w", stdout);
#endif
	...
	while (T--) {
		...
		auto calc = [&scm](map<char, ll> &cc) {
			ll ans = 0;
			for (auto it : cc) {
				ans += it.second * scm[it.first];
			}
			return ans;
		};
		map<char, ll> cc;
		...
		for (ll i = 0; i < s.size(); i++) {
			...
			cc[s[i]]++;
			if (s.size() == calc(cc)) {
				auto cand = s.substr(0, i + 1);
				string candt = cand;
				for (auto &c : sc) {
					cand.erase(remove(cand.begin(), cand.end(), c), cand.end());
					candt += cand;
				}
				if (candt == s) {
					cout << s.substr(0, i + 1) << ' ' << sc << '\n';
					done = true;
					break;
				}
			}
