...
using MF = Rain::Math::ModulusField<LL, 1000000007>;

int main() {
	ios_base::sync_with_stdio(false);
	cin.tie(nullptr);

	MF::precomputeFactorials(500000);
	...
	while (_T--) {
		...
		while (!Q.empty()) {
			LL i{Q.front()};
			Q.pop();
			auto ji{LVM.find(i)}, ki{ji};
			ji--;
			ki++;
			LL j{*ji}, k{*ki}, xl(sqrt(k - j - 2));
			if (j + 1 < i) {
				LL cl{j + 1};
				fail = true;
				LL sc{};
				for (auto si{--AS.end()}; sc < xl; si--, sc++) {
					if (si->second <= j || si->second >= i) {
						continue;
						;
					}
					if (
						si->first ==
						(si->second - j) * (i - si->second)) {
						fail = false;
						cl = si->second;
					}
					if (si == AS.begin()) {
			...
			if (i + 1 < k) {
				...
				LL sc{};
				for (auto si{--AS.end()}; sc < xl; si--, sc++) {
					if (si->second <= i || si->second >= k) {
						continue;
						;
					}
					if (
						si->first ==
						(si->second - i) * (k - si->second)) {
						fail = false;
						cl = si->second;
					}
					if (si == AS.begin()) {
				...
				AS.erase({A[cl], cl});
				if (k - i - 2 > 0) {
					Z *= MF{k - i - 2}.choose(cl - i - 1);
				}
			}
		}
		// RF(i, 0, N + 2) { cout << LV[i] << ' '; }
		// cout << '\n';
		if (fail) {
			cout << "0\n";
			continue;
