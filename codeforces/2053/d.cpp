...
int main() {
	...
	while (T--) {
		...
		RF(i, 0, Q) {
			...
			if (O == 1) {
				X = RA[X];
				auto F{upper_bound(A.begin(), A.end(), pair<LL, LL>(A[X].first, N)) -
					A.begin()};
				F--;
				Z /= min(B[X].first, A[X].first);
				if (F != X) {
					Z /= min(B[F].first, A[F].first);
				}
				A[X].first++;
				swap(A[X], A[F]);
				swap(RA[A[X].second], RA[A[F].second]);
				Z *= min(B[X].first, A[X].first);
				if (F != X) {
					Z *= min(B[F].first, A[F].first);
				}
				...
				X = RB[X];
				auto F{upper_bound(B.begin(), B.end(), pair<LL, LL>(B[X].first, N)) -
					B.begin()};
				F--;
				Z /= min(B[X].first, A[X].first);
				if (F != X) {
				...
				B[X].first++;
				swap(B[X], B[F]);
				swap(RB[B[X].second], RB[B[F].second]);
				Z *= min(B[X].first, A[X].first);
				if (F != X) {
					Z *= min(B[F].first, A[F].first);
				}
