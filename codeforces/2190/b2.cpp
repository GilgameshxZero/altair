...
namespace Rain::Algorithm {
	...
	class ModulusRingBase {
		...
		// value. Construction with another ModulusRingBase will
		// call the % operator on the other's value, which will
		// always stay in range of the other's ring. This value
		// is then explicitly cast into the current underlying
		// type.
		//
...
int main() {
	...
	while (_T--) {
		...
		C[0] = 1;
		RF(i, 0, N) {
			array<MF, 101> sl{SL}, c;
			LL diff{S[i] == '(' ? 1LL : -1LL};
			RF(j, 0, c.size()) {
				c[j] = C[j];
				if (j - diff >= 0 && j - diff < (LL)c.size()) {
					c[j] += C[j - diff];
					sl[j] += C[j - diff] + SL[j - diff];
				}
			}
			swap(sl, SL);
			swap(c, C);
		...
		C[0]--;
		// cout << C[0] << ',' << SL[0] << ',' << SL[0] - 2 * C[0]
		// 		 << '\n';
		MF Z{SL[0] - 2 * C[0]}, YC1, YC2;
		RF(i, 0, N) {
			if (S[i] == '(') {
				continue;
			}
			...
			RF(j, 0, min(AV[1], AV[0] - 1) + 1) {
				YC1 +=
					AV[0].choose(j + 1) * AV[1].choose(j) * (j * 2);
			}
		}
		RF(i, 0, N) {
			if (S[i] == ')') {
			...
			RF(j, 0, i) {
				...
				RF(k, 0, AV[1] + 1) {
					RF(l, 1, AV[2] + 1) {
						...
						// cout << '(' << i << ',' << j << ',' << k << ','
						// 		 << l << ')' << '\n';
						YC2 += AV[0].choose(k + l) * AV[1].choose(k) *
							AV[2].choose(l) * ((k + l) * 2);
					}
				}
			}
