...
MF spc(vector<pair<LL, LL>> &transpose, LL remsum, LL cur, LL K) {
	if (K <= 0) {
		return 1;
	}
	if (K > remsum) {
	...
	MF ans{0}, ss{1};
	RF(i, 0, transpose[cur].second + 1) {
		if (K - i < 0) {
			break;
		}
		ans += MF(transpose[cur].second).choose(i) * spc(transpose, remsum - transpose[cur].second, cur + 1, K - i) * ss;
		ss *= transpose[cur].first;
	}
	return ans;
}
...
int main(int, char const *[]) {
	...
	minFactor[1] = 0;
	MF::precomputeFactorials(4044);

	LL N;
	cin >> N;
	unordered_map<LL, LL> mults;
	RF(i, 0, 2 * N) {
		LL X;
		cin >> X;
		mults[X]++;
	}
	MF basic{MF::factorials[N]};
	unordered_map<LL, LL> final;
	for (auto const &i : mults) {
		basic *= MF::invFactorials[i.second];
		if (minFactor[i.first] == i.first) {
			final[i.second]++;
		}
	}
	vector<pair<LL, LL>> transpose;
	LL remsum{0};
	for (auto const &i : final) {
		transpose.push_back(i);
		remsum += i.second;
	}
	cout << spc(transpose, remsum, 0, N) * basic;
