...
int main(int argc, char const *argv[]) {
	...
	while (T--) {
		...
		string S, T;
		cin >> N >> S >> T;
		AR<LL, 26> snp{0}, mms{0};
		VR<bool> moved(N, false);
		LL ans = LLONG_MAX, carry = 0, mmi = 0;
		for (LL i = 0, k = 0; i < N && k < N; i++, k++) {
			while (moved[i]) {
				i++;
				mmi++;
			}
			...
			if (S[i] >= T[k]) {
				...
				RF(j, 0, sameid) {
					for (; snp[j] < N; snp[j]++) {
						...
						if (snp[j] >= i + 1 && S[snp[j]] - 'a' == j) {
							break;
						}
					}
					if (snp[j] < N) {
						best = min(best, snp[j] - i - mms[j] + mmi);
					}
				}
				if (best != LLONG_MAX) {
					ans = min(ans, carry + best);
				}
			}
			if (S[i] > T[k]) {
				LL sameid = T[k] - 'a';
				for (; snp[sameid] < N; snp[sameid]++) {
					while (snp[sameid] < N && moved[snp[sameid]]) {
						snp[sameid]++;
						mms[sameid]++;
					}
					if (snp[sameid] >= N) {
					...
					if (snp[sameid] >= i + 1 && S[snp[sameid]] - 'a' == sameid) {
						break;
					}
				}
				if (snp[sameid] < N) {
					carry += snp[sameid] - i - mms[sameid] + mmi;
					moved[snp[sameid]] = true;
					RF(j, 0, 26) {
						if (snp[j] > snp[sameid]) {
							mms[j]++;
						}
