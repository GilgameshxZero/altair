...
int main(int argc, char const *argv[]) {
	...
	while (T--) {
		...
		while (ans.size() != N) {
			...
			for (auto it : ranges) {
				if (
					it.first <= start.first && it.second >= start.second &&
					it.second - it.first + 1 < best.second - best.first + 1) {
					best = it;
				}
			}
			if (start.first == best.first) {
				ans[best] = start.second + 1;
			}
			if (start.second == best.second) {
				ans[best] = start.first - 1;
			}
		}
