...
int main(int argc, char const *argv[]) {
	...
	while (T--) {
		LL N;
		cin >> N;
		VR<PR<bool, PR<LL, LL>>> songs(N);
		RF(i, 0, N) {
			cin >> songs[i].second.first;
			songs[i].second.first--;
			songs[i].second.second = i;
		}
		string S;
		cin >> S;
		RF(i, 0, N) { songs[i].first = S[i] == '1'; }
		std::sort(songs.begin(), songs.end());
		VR<LL> ratings(N);
		RF(i, 0, N) { ratings[songs[i].second.second] = i; }
		RF(i, 0, N) { cout << ratings[i] + 1 << ' '; }
		cout << '\n';
	}
