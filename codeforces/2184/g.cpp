...
namespace Rain::Algorithm {
	...
	class SegmentTreeLazy<std::nullptr_t> {
			...
			std::string message(int error) const noexcept {
				switch (error) {
					...
					case Error::NOT_IMPLEMENTED_POLICY:
						return "An unimplemented Policy function was "
									 "called.";
		...
		public:
		// Default policy for SegmentTreeLazy, which has
		// functions disabled by SFINAE if they do not support
		...
		class PolicyBase
			...
			// Some functions do not need to be provided by the
			// policy if the segtree does not use them.
			//
		...
		// Wraps a policy to implement a persistent segment tree
		// via the fat-node technique. In fate node, only the
		// latest copy is modifiable.
		//
		// To apply updates out-of-order, we recommend the
		// client apply updates and queries in reverse-time
		// order. In this way, applying an earlier update will
		// apply it correctly, but also invalidate later,
		// already-applied "version"s of the tree.
		//
		// Lazy range updates are supported by persisting lazy
		// updates. However, this wrapper does not support it,
		// as there is no query information provided in
		// `retrace` and `apply`, which is necessary. To support
		// persistent lazy updates, we prefer a separate path
		// copying alternative segment tree implementation
		// (TODO).
		//
		// A query for time `t` is evaluated after all requested
		// updates at time `t` have been applied.
		//
		...
		class PolicyPersistentFatNodeWrapper {
			public:
			using Value =
				std::map<TimeType, typename Policy::Value>;
			using Update =
				std::pair<TimeType, typename Policy::Update>;
			using Result = typename Policy::Result;
			using Query =
				std::pair<TimeType, typename Policy::Query>;
			...
			// TODO: Persistent retrace performs redundant work in
			// binary searching left/right each time, so this is
			// slower than necessary by a constant factor.
			static inline void retrace(
				Value &value,
				Value const &left,
				Value const &right,
				Update const &update) {
				auto hint{
					std::prev(value.upper_bound(update.first))};
				auto it{value.insert_or_assign(
					hint, update.first, hint->second)};
				Policy::retrace(
					it->second,
					std::prev(left.upper_bound(update.first))->second,
					std::prev(right.upper_bound(update.first))
						->second,
					update.second);
			}
			static inline void build(
				...
				Value const &right) {
				auto t{std::max(
					left.rbegin()->first, right.rbegin()->first)};
				auto hint{std::prev(value.upper_bound(t))};
				auto it{
					value.insert_or_assign(hint, t, hint->second)};
				Policy::build(
					it->second,
					left.rbegin()->second,
					right.rbegin()->second);
			...
			static inline Result aggregate(
				Result const &left,
				Result const &right,
				Query const &query) {
		...
		class PolicySum2DPoint
			...
			// combine is omitted because we only support point
			// updates.
			static inline void retrace(
				Value &value,
				Value const &,
			...
			// build is omitted because there is no easy way to
			// combine two Fenwicks.
			static inline void apply(
				Value &value,
				Update const &update,
				std::size_t) {
	...
	// Segment tree with lazy propagation, supporting range
	// queries and range updates in O(ln N) and O(N) memory.
	//
	// Loosely based on
	// <https://codeforces.com/blog/entry/18051>. Earlier
	// iterations of this data structure have higher constant
	// factor but enable more intuitive modifications. Due to
	// the memory layout, some vertices are "bridge" vertices,
	// and aggregate a prefix and suffix of the underlying
	// together. Bridges do not have a uniform height or size.
	//
	// Inherit and modify the provided default policy to
	// implement custom behavior.
	//
	// Updates must be combinable. If updates are not
	// combinable, consider using the non-lazy version of this
	// segtree, or otherwise using a higher-dimensional
	// structure (e.g. quad-tree). Otherwise, using only point
	// updates will guarantee that combine is never called.
	//
	// build is only required when using the building version
	// of the constructor. retrace receives information about
	...
	class SegmentTreeLazy {
		...
		class Vertex {
			...
			// True iff node has a pending lazy update to
			// propagate to its children. The update has already
			// been applied to the node itself.
			bool lazy{false};

			// Lazily-stored updates. Will be the default update
			// if lazy is false.
		...
		// Propagate a single non-leaf vertex in the tree. It is
		// guaranteed that propagate will never be called on a
		// leaf. propagate may be called on a bridge, but
		// bridges (and ancestors of them) will never be lazy.
		//
		...
		inline void propagate(std::size_t idx, std::size_t size)
			...
			if (idx * 2 < this->SIZE_UNDERLYING) {
				Policy::combine(
					this->vertices[idx * 2].update,
					this->vertices[idx].update);
				Policy::combine(
					this->vertices[idx * 2 + 1].update,
					this->vertices[idx].update);
				this->vertices[idx * 2].lazy =
					this->vertices[idx * 2 + 1].lazy = true;
			}
		...
		public:
		// Publicly accessible way to traverse the tree while
		// abstracting away the lazy propagation. It is assumed
		// that the tree has been propagated to the iterator
		// already.
		//
		// Iterators remain valid as long as no operations are
		// performed between accesses. Operations may invalidate
		// the values of iterators as they will no longer be
		// up-to-date with lazy updates. This can be resolved
		// with `revalidate`.
		//
		...
		class Iterator {
			...
			public:
			// Size is the number of leaf nodes in this subtree.
			// It is only valid if this is not an ancestor of a
			// bridge.
			Iterator(
				SegmentTreeLazy<Policy> const *tree,
				std::size_t idx,
			...
			inline Iterator right() {
				...
				this->TREE->propagate(this->IDX, this->SIZE);
				return {
					this->TREE, this->IDX * 2 + 1, this->SIZE / 2};
			}
			// Underlying access. Throws if at the ends of the
			// underlying array, or if this is not leaf. Because
			// of the need to propagate, these are O(\lg N)
			// amortized.
			//
			...
			// If updates have changed the tree, re-validate this
			// iterator by propagating to it.
			inline void revalidate() {
				this->TREE->propagateTo(this->IDX);
			}

			// Access to iterator value.
			Value &operator*() {
				return this->TREE->vertices[this->IDX].value;
			}
		};
		...
		// Queries an inclusive range, propagating if necessary
		// then aggregating.
		Result query(
			std::size_t left,
			std::size_t right,
			...
			for (left += this->SIZE_UNDERLYING,
				...
				if (left % 2 == 1) {
					resLeft = Policy::aggregate(
						resLeft,
						Policy::convert(
							this->vertices[left++].value, query, size),
						query);
				}
				if (right % 2 == 1) {
					resRight = Policy::aggregate(
						Policy::convert(
							this->vertices[--right].value, query, size),
						resRight,
						query);
				}
			}
		...
		// Lazy update an inclusive range. The updated will be
		// applied identically to all nodes in the range, save
		// for differences based on the depth of the node (which
		// will be expressed via the std::size_t range
		// parameter).
		void update(
			std::size_t left,
			std::size_t right,
			...
			this->propagateTo(left + this->SIZE_UNDERLYING);
			this->propagateTo(right + this->SIZE_UNDERLYING);
			// Only retrace updates once left or right node has
			// been changed.
			...
			for (left += this->SIZE_UNDERLYING,
				...
				if (left % 2 == 1) {
					changedLeft = true;
					Policy::apply(
						this->vertices[left].value, update, size);
					// Avoid unnecessarily setting the lazy update on
					// a leaf node.
					if (left < this->SIZE_UNDERLYING) {
						Policy::combine(
							this->vertices[left].update, update);
				...
				if (right % 2 == 1) {
					...
					changedRight = true;
					Policy::apply(
						this->vertices[right].value, update, size);
					if (right < this->SIZE_UNDERLYING) {
						Policy::combine(
							this->vertices[right].update, update);
			...
			for (left--; right > 0; left /= 2, right /= 2) {
				...
				if (
					changedRight && (!changedLeft || left != right)) {
					Policy::retrace(
						this->vertices[right].value,
						this->vertices[right * 2].value,
						this->vertices[right * 2 + 1].value,
		...
		Iterator backUnderlying() {
			...
			return Iterator(
				this, this->SIZE_UNDERLYING * 2 - 1, 1);
		}
	};
...
int main() {
	...
	while (_T--) {
		...
		SegmentTreeLazy<SegmentTreeLazy<>::PolicyMin<LL>> T(N);
		RF(i, 0, N) {
			LL X;
			cin >> X;
			T.update(i, i, X);
