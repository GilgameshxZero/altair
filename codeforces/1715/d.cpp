...
int main(int, char const *[]) {
	...
	RF(i, 0, Q) {
		LL I, J, X;
		cin >> I >> J >> X;
		e[I - 1].push_back({J - 1, X});
		e[J - 1].push_back({I - 1, X});
	}
	RF(i, 0, 30) {
		...
		RF(j, 0, N) {
			if (zero.find(j) != zero.end() || ((A[j] & (1LL << i)) != 0)) {
				continue;
			}
			RF(k, 0, e[j].size()) {
				if (
					e[j][k].first <= j && ((e[j][k].second & (1LL << i)) != 0) &&
					((A[e[j][k].first] & (1LL << i)) == 0)) {
					A[j] |= (1LL << i);
					break;
				}
			}
