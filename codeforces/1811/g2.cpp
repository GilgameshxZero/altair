...
// // Hash operator for this user-defined type, which hashes the inner value
// (not
// // the modulus).
// namespace std {
// 	template <typename Integer, std::size_t MODULUS_OUTER>
	...
	while (T--) {
		...
		RF(i, 1, N) {
			...
			RF(j, i - 1, -1) {
				csame += C[j] == C[i];
				if (csame == K) {
					blocks[i] = max(blocks[i], j == 0 ? 1LL : blocks[j - 1] + 1);
					break;
				}
			}
		}
		...
		RF(i, 1, N) {
			LL csame{1};
			ways[i] = blocks[i - 1] == blocks[i] ? ways[i - 1] : MF{0};
			RF(j, i - 1, -1) {
				if ((j == 0 ? 1LL : blocks[j - 1]) < blocks[i] - 1) {
					break;
				}
				if (C[j] == C[i]) {
					csame++;
					if (csame >= K - 1) {
						ways[i] +=
							(j == 0 ? MF{1} : ways[j - 1]) * MF{csame - 2}.choose(K - 2);
					}
				}
			}
		}
