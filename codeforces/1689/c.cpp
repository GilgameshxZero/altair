...
PR<LL, LL> dfs(VR<VR<LL>> &edge, LL x, LL par) {
	LL children{0};
	RF(i, 0, edge[x].size()) {
		if (edge[x][i] == par) {
			continue;
	...
	if (children == 0) {
		...
		auto sub{dfs(edge, child, x)};
		return {sub.first + 1, sub.first - 1};
	} else {
		VR<LL> cx;
		RF(i, 0, edge[x].size()) {
			if (edge[x][i] == par) {
			...
			cx.push_back(edge[x][i]);
		}
		auto sub1{dfs(edge, cx[0], x)}, sub2{dfs(edge, cx[1], x)};
		return {
			sub1.first + sub2.first + 1,
			max(sub1.first - 1 + sub2.second, sub1.second + sub2.first - 1)};
	}
}
