...
ll evRot(vector<ll> &rots, vector<ll> &A, ll D, ll x) {
	if (rots[x] != -1) {
		return rots[x];
	}
	if (A[x] == 0) {
	...
	rots[x] = LLONG_MAX;
	ll recEv = evRot(rots, A, D, (x + A.size() - D) % A.size());
	if (recEv == LLONG_MAX) {
		return rots[x] = LLONG_MAX;
	}
	return rots[x] = 1 + recEv;
}
