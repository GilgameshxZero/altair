...
bitset<200000> isPrime;
VR<LL> primes;
unordered_map<LL, LL> rPrimeMap;

void gather(VR<LL> &factors, VR<VR<PR<LL, PR<LL, LL>>>> &edge, LL par, LL cur) {
	for (auto &i : edge[cur]) {
		if (i.first == par) {
			continue;
		}
		LL x{i.second.first}, y{i.second.second};
		unordered_map<LL, LL> delta;
		RF(j, 0, primes.size()) {
		...
		for (auto &j : delta) {
			factors[j.first] = max(0LL, factors[j.first] + j.second);
		}
		gather(factors, edge, cur, i.first);
		for (auto &j : delta) {
			factors[j.first] -= j.second;
		}
	}
}
...
void spread(
	...
	for (auto &i : edge[cur]) {
		...
		spread(sum, prod * i.second.second / i.second.first, edge, cur, i.first);
	}
}
...
int main(int, char const *[]) {
	...
	while (T--) {
		LL N;
		cin >> N;
		VR<VR<PR<LL, PR<LL, LL>>>> edge(N);
		RF(i, 0, N - 1) {
			LL j, k, x, y;
			cin >> j >> k >> x >> y;
			edge[j - 1].push_back({k - 1, {x, y}});
			edge[k - 1].push_back({j - 1, {y, x}});
		}

		VR<LL> factors(primes.size(), 0);
		gather(factors, edge, -1, 0);
		MR sum{0}, prod{1};
		RF(i, 0, factors.size()) { prod *= MR::power(primes[i], factors[i]); }
		spread(sum, prod, edge, -1, 0);
		cout << sum << '\n';
	}
