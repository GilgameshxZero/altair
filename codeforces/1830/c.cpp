...
int main(int, char const *[]) {
	...
	vector<MF> V(150000);
	MF::precomputeFactorials(V.size() * 2);
	V[0] = 1;
	RF(i, 1, V.size()) {
		V[i] = MF{2 * i}.choose(i) - MF{2 * i}.choose(i + 1);
		// RF(j, 0, i) {
		// 	V[i] += V[j] * V[i - 1 - j];
		// }
	}
	...
	while (T--) {
		...
		while (S.size() > 1) {
			...
			if (i.first.second <= j.first.first) {
				Y[i.second] += i.first.second - i.first.first;
			} else if (j.first.second < i.first.second) {
				S.push({{j.first.second, i.first.second}, i.second});
				Y[i.second] += j.first.first - i.first.first;
			} else {
				S.pop();
				S.push({{i.first.second, j.first.second}, j.second});
				S.push({{j.first.first, i.first.second}, (LL)Y.size()});
				Y.push_back(0);
				Y[i.second] += j.first.first - i.first.first;
			}
		}
