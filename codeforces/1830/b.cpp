...
using HT = __gnu_pbds::gp_hash_table<Key, Value, Hash>;
#else
template <typename Key, typename Value, typename Hash>
using HT = unordered_map<Key, Value, Hash>;
#endif
...
int main(int, char const *[]) {
	...
	while (T--) {
		LL N;
		cin >> N;
		LL Q{(LL)sqrt(2 * N) + 1}, Z{0}, MM{0};
		RF(i, 1, Q + 1) {
			RF(j, 1, N + 1) {
				M[i][j] = -1;
			}
		}
		vector<LL> A(N), B(N), I(N, -1);
		RF(i, 0, N) {
			cin >> A[i];
		}
		RF(i, 0, N) {
