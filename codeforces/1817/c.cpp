...
int main(int, char const *[]) {
	...
	MF::precomputeFactorials(2500000);
	...
	MF ADB{0}, ADM{0}, BDB{0};
	RF(i, 0, D) {
		ADB += A[i] * MF{D - 1}.choose(i) * (i % 2 == 0 ? -1 : 1);
		ADM += A[i + 1] * MF{D - 1}.choose(i) * (i % 2 == 0 ? -1 : 1);
		BDB += B[i] * MF{D - 1}.choose(i) * (i % 2 == 0 ? -1 : 1);
	}
	if (D % 2 == 1) {
		ADB *= -1;
		ADM *= -1;
