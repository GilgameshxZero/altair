...
int main(int, char const *[]) {
	...
	while (T--) {
		...
		for (auto i = C.begin(); i != C.end(); i++) {
			occ.push_back({i->first * i->second, i->first});
		}
		sort(occ.begin(), occ.end());
		unordered_map<LL, unordered_set<LL>> bad;
		RF(i, 0, M) {
			LL t1, t2;
			cin >> t1 >> t2;
			if (bad.find(t1) == bad.end()) {
				bad[t1] = {};
			}
			bad[t1].insert(t2);
			if (bad.find(t2) == bad.end()) {
				bad[t2] = {};
			}
			bad[t2].insert(t1);
		}
		RF(i, 0, occ.size()) {
			if (bad.find(occ[i].second) == bad.end()) {
				bad[occ[i].second] = {};
			}
		}
		...
		LL ans = 0;
		RF(i, 0, occ.size()) {
			LL x = occ[i].second, cx = occ[i].first / occ[i].second;
			unordered_set<LL> const &bx = bad[x];
			if (bx.size() == occ.size()) {
				continue;
			}
			LL j;
			for (j = occ.size() - 1; j >= 0; j--) {
				if (bx.find(occ[j].second) == bx.end()) {
					break;
				}
			}
			LL y = occ[j].second, cy = occ[j].first / occ[j].second;
