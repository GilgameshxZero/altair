...
int main(int, char const *[]) {
	...
	while (T--) {
		...
		LL A = 0, B = 0;
		VR<PR<LL, LL>> P(N);
		RF(i, 0, N) { cin >> P[i].first; }
		RF(i, 0, N) { cin >> P[i].second; }
		dp[0][10000] = 4;
		RF(i, 1, N + 1) {
			RF(j, 0, 20000) {
				if (dp[i - 1][j] != 0) {
					dp[i][j + P[i - 1].second - P[i - 1].first] |= 1;
					dp[i][j + P[i - 1].first - P[i - 1].second] |= 2;
				}
			}
		}
		...
		VR<bool> unwrap;
		while(unwrap.size() < N) {
			if (dp[N - unwrap.size()][best] & 1) {
				unwrap.push_back(false);
				best -= P[N - unwrap.size()].second - P[N - unwrap.size()].first;
			} else {
				unwrap.push_back(true);
				best -= P[N - unwrap.size()].first - P[N - unwrap.size()].second;
			}
		}
		reverse(unwrap.begin(), unwrap.end());

		LL ans = 0;
		RF(i, 0, N) {
			RF(j, i + 1, N) {
				LL x1 = P[i].first, y1 = P[i].second, x2 = P[j].first, y2 = P[j].second;
				if (unwrap[i]) {
					swap(x1, y1);
				}
				if (unwrap[j]) {
