	...
	for (ll t = 0; t < T; t++) {
		...
		for (ll n = 0; n < N; n++) {
			...
			for (auto &wit : work) {
				...
				while (nwit != nw.end() && nwit->first < wit.first) {
					nwit++;
				}
				while (nwit != nw.end() && nwit->first == wit.first &&
							 nwit->second != -1) {
				...
				if (
					nwit != nw.end() && nwit->first == wit.first && nwit->second == -1) {
					nwit->second = wit.second;
					wit.second = -1;
				}
			}
			nwit = nw.begin();
			...
			for (auto &wit : work) {
				...
				while (nwit != nw.end() && nwit->second != -1) {
					nwit++;
				}
				nwit->second = wit.second + 1;
				nwit++;
			}
			swap(work, nw);
		}
