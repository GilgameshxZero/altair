...
int main(int, char const *[]) {
	...
	while (T--) {
		...
		vector<unordered_map<LL, MF>> E(N), rE(N);
		...
		while (!Q.empty()) {
			...
			Q.pop();
			for (auto &i : E[cur]) {
				for (auto const &j : rec[cur]) {
					rec[i.first][j.first + 1] += j.second;
				}
		...
		RF(i, 0, N) {
			if (E[i].empty()) {
				LL cap{0}, prev{0};
				for (auto const &j : rec[i]) {
					cap = max(0LL, cap - (j.first - prev));
					cap += j.second;
					prev = j.first;
				}
				cout << MF{cap + rec[i].rbegin()->first} << '\n';
				break;
			}
		}
	}
