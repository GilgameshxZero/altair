...
class AddMinTreePolicy {
	public:
	using Value = LL;
	using Update = LL;
	using Result = LL;

	static constexpr Value DEFAULT_VALUE{LLONG_MAX / 3};
...
int main() {
	...
	SegmentTreeLazy<AddMinTreePolicy> T(K + 1);
	T.update(0, 0, -LLONG_MAX / 3);
	RF(i, 1, K + 1) {
		LL W{T.query(i - 1, i - 1) + (i - 1) * A}, V{0};
		RF(j, 0, I[K - i].size()) {
			T.update(I[K - i][j].first, i - 1, I[K - i][j].second);
			V += I[K - i][j].second;
		}
		LL X{T.query(0, i - 1) + (i - 1) * A};
