...
int main(int, char const *[]) {
	...
	while (T--) {
		...
		cin >> N >> M;
		string B;
		PR<LL, LL> ul{N, M}, ur{N, 1}, bl{1, M}, br{1, 1};
		RF(i, 1, N + 1) {
			cin >> B;
			RF(j, 1, M + 1) {
				if (B[j - 1] == 'B') {
					if (i + j < ul.first + ul.second) {
						ul = {i, j};
					}
					if (i - j < ur.first - ur.second) {
						ur = {i, j};
					}
					if (i - j > bl.first - bl.second) {
						bl = {i, j};
					}
					if (i + j > br.first + br.second) {
						br = {i, j};
					}
				}
			}
		...
		PR<LL, LL> mm{
			ul.first + ul.second + br.first + br.second,
			ur.first - ur.second + bl.first - bl.second},
			rc{(mm.first + mm.second + 2) / 4, (mm.first - mm.second + 2) / 4};
