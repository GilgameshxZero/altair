...
int main(int, char const *[]) {
	...
	while (T--) {
		...
		LL D;
		cin >> S >> D;
		LL s{stoll(S.substr(0, 2)) * 60 + stoll(S.substr(3, 2))};
		LL ans{0};
		set<LL> vis;
		while (vis.find(s) == vis.end()) {
			vis.insert(s);
			LL hour{s / 60}, minute{s - hour * 60};
			string shour{to_string(hour)}, sminute{to_string(minute)};
			if (shour.length() < 2) {
				shour = "0" + shour;
			}
			if (sminute.length() < 2) {
