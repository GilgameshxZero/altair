...
AR<PR<LL, LL>, 300> A;
AR<AR<bool, 300>, 300> done;

int main(int, char const *[]) {
	LL N, K;
	cin >> N >> K;
	RF(i, 0, N) { cin >> A[i].first >> A[i].second; }
	if (K == 1) {
		cout << "Infinity";
		return 0;
	}
	...
	RF(i, 0, N) {
		RF(j, i + 1, N) {
			...
			RF(k, 0, N) {
				...
				PR<LL, LL> s1{X.first - A[k].first, X.second - A[k].second},
					s2{Y.first - A[k].first, Y.second - A[k].second};
			...
			RF(k, 0, col.size()) {
				RF(l, 0, col.size()) { done[col[k]][col[l]] = true; }
			}
		}
	}
	cout << ans;
