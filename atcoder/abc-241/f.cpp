...
int main(int, char const *[]) {
	...
	RF(i, 0, N) {
		cin >> X >> Y;
		row[X - 1].push_back(Y - 1);
		col[Y - 1].push_back(X - 1);
	}
	priority_queue<PR<LL, PR<LL, LL>>> pq;
	set<PR<LL, LL>> vis;
	pq.push({0, S});
	while (!pq.empty()) {
		auto top = pq.top().second;
		auto moves = -pq.top().first;
		pq.pop();
		...
		auto i =
			upper_bound(row[top.first].begin(), row[top.first].end(), top.second);
		auto j =
			upper_bound(col[top.second].begin(), col[top.second].end(), top.first);
		LL prev, next;
		if (i == row[top.first].begin()) {
			prev = 0;
		} else {
			auto ii = i;
			ii--;
		...
		if (i == row[top.first].end()) {
			next = W - 1;
		} else {
			auto ii = i;
			prev = *ii - 1;
		}
		pq.push({-moves - 1, {top.first, prev}});
		pq.push({-moves - 1, {top.first, next}});
		if (j == col[top.second].begin()) {
			prev = 0;
		} else {
			auto jj = j;
			jj--;
		...
		if (j == col[top.second].end()) {
			next = H - 1;
		} else {
			auto jj = j;
			next = *jj - 1;
		}
		pq.push({-moves - 1, {prev, top.second}});
		pq.push({-moves - 1, {next, top.second}});
	}
