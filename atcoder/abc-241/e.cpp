...
int main(int, char const *[]) {
	...
	while (!vis[X.back() % N]) {
		vis[X.back() % N] = true;
		X.push_back(X.back() + A[X.back() % N]);
	}
	LL crux{X.back() % N}, pre{0}, prec{0}, sum{0}, cycle{0};
	bool isPrefix{true};
	X.pop_back();
	RF(i, 0, X.size()) {
		if (X[i] % N == crux) {
	...
	if (K < prec) {
		LL ans{0};
		RF(i, 0, K) { ans += A[X[i] % N]; }
		cout << ans;
		return 0;
	}
	LL rem = (K - prec) % cycle, cc = (K - prec) / cycle, ans{cc * sum + pre};
	RF(i, prec, prec + rem) { ans += A[X[i] % N]; }
	cout << ans;
