...
auto next(VR<LL> &A, LL X, LL i) {
	LL ans = 0;
	RF(j, (LL)A.size() - 1, i - 1) {
		LL k = X / A[j];
		ans += k * A[j];
		X -= k * A[j];
	}
...
void dfs(VR<LL> &A, LL x, LL i, LL j, LL &ans) {
	if (j >= ans) {
		return;
	}
	ans = min(ans, count(A, x) + j);
	if (i + 1 < A.size()) {
		LL rem = x % A[i + 1];
		if (rem <= A[i + 1] - rem) {
			dfs(A, x - rem, i + 1, j + rem / A[i], ans);
		}
		if (rem >= A[i + 1] - rem) {
			dfs(A, x + A[i + 1] - rem, i + 1, j + (A[i + 1] - rem) / A[i], ans);
		}
	}
}
