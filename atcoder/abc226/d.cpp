...
int main(int argc, char const *argv[]) {
	struct cmp {
		bool operator()(PR<LL, LL> const &L, PR<LL, LL> const &R) const {
	...
	cin >> N;
	VR<PR<LL, LL>> Ts(N);
	RF(i, 0, N) { cin >> Ts[i].first >> Ts[i].second; }
	set<PR<LL, LL>, cmp> slopes;
	RF(i, 0, N) {
		RF(j, i + 1, N) {
			slopes.insert({Ts[i].first - Ts[j].first, Ts[i].second - Ts[j].second});
			slopes.insert({Ts[j].first - Ts[i].first, Ts[j].second - Ts[i].second});
		}
	}
	cout << slopes.size();
