...
LL color(LL scheme, LL block, LL it) {
	if (scheme == 0) {
		return S[block][it];
	} else if (scheme == 1) {
		return S[it][block];
		...
		if (block < N) {
			return S[it][N - 1 - block + it];
		} else {
			return S[block - (N - 1) + it][it];
		}
	} else if (scheme == 3) {
		if (block < N) {
			return S[it][block - it];
		} else {
			return S[block - (N - 1) + it][N - 1 - it];
		}
	}
}
