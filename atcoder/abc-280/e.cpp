...
int main(int, char const *[]) {
	...
	MR ans{0}, pOnes, pTwos{1}, pTwosMul{P}, pOnesDiv{100 - P}, pOnesMul;
	pOnesDiv /= 100;
	pOnes = MR::power(pOnesDiv, N);
	pOnesMul = pOnesDiv;
	pOnesDiv = 1 / pOnesDiv;
	pTwosMul /= 100;
	RF(i, 0, (N + 1) / 2 + 1) {
		LL twos{i}, ones{N - 2 * i};
		if (ones < 0) {
			ans += 1 * pTwos * twos * fact[twos] / fact[0] / fact[twos];
		} else {
			ans += pOnes * pTwos * (ones + twos) * fact[ones + twos] / fact[ones] /
				fact[twos];
			if (twos > 0) {
				ones++;
				pOnes *= pOnesMul;
				ans += pOnes * pTwos * (ones + twos) * fact[ones + twos - 1] /
					fact[ones] / fact[twos - 1];
				pOnes *= pOnesDiv;
			}
			pOnes *= pOnesDiv * pOnesDiv;
			pTwos *= pTwosMul;
		}
	}
	cout << ans;
