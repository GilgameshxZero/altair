...
#define RF(x, from, to) \
	for (long long x = from, _rfDir = x < to ? 1 : -1; x != to; x += _rfDir)
...
int main(int argc, char const *argv[]) {
	LL N, W;
	cin >> N >> W;
	VR<PR<LL, LL>> cheese(N);
	RF(i, 0, N) { cin >> cheese[i].first >> cheese[i].second; }
	sort(cheese.begin(), cheese.end());

	LL ans = 0;
	RF(i, cheese.size() - 1, -1) {
		LL sub = min(W, cheese[i].second);
		W -= sub;
		ans += sub * cheese[i].first;
	}
	cout << ans;
