...
ll llpow(ll b, ll p) {
	return p == 0 ? 1 : b * llpow(b, p - 1);
}

int main(int argc, char const *argv[]) {
	// Redirect I/O to/from files if running locally.
#ifndef ONLINE_JUDGE
	std::freopen("f-in.txt", "r", stdin);
	std::freopen("f-out.txt", "w", stdout);
#endif
	...
	while (T--) {
		...
		string sc = s;
		for (ll lic = li; lic < s.size(); lic++) {
			sc[lic] = *kbc.rbegin();
		}

		ll sn = strtoll(s.c_str(), nullptr, 10),
			 scn = strtoll(sc.c_str(), nullptr, 10);
		if (sc < s) {
			kbc.clear();
			ll snc =
				sn - (sn % llpow(10, s.size() - li)) + llpow(10, s.size() - li);
			string ss = to_string(snc);
			ll lii;
			for (lii = 0; lii < ss.size(); lii++) {
				if (kbc.size() == k) {
					break;
				}
				kbc.insert(ss[lii]);
			...
			sc = s;
			for (ll lii = li; lii < sc.size(); lii++) {
				sc[lii] = *kbc.begin();
			}
			while (sc < s) {
				for (ll lii = sc.size() - 1; lii >= li; lii--) {
					auto it = kbc.find(sc[lii]);
					if (++it != kbc.end()) {
						sc[lii] = *it;
						break;
					} else {
