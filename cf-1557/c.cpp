...
size_t modPow(unsigned int base, long long power) {
	base %= MOD;
	power %= (MOD - 1);
	if (power == 0) {
		return 1;
	} else if (power < 0) {
		return modPow(base, power * (MOD - 2));
	} else if (power % 2 == 0) {
		size_t halfPow = modPow(base, power / 2);
...
#pragma warning(default : 6031)
	...
	while (T--) {
		...
		if (N % 2 == 0) {
			cout << ((((modPow(2, N * K) *
									(1 + MOD - modPow(modPow(2, -1) - 1 + MOD, K)) % MOD) *
								 modPow(modPow(2, N - 1) + 1, -1)) %
								MOD) *
							 2) %
					MOD
					 << '\n';
		} else {
			cout << (1 + modPow(2, K * N - K)) % MOD << '\n';
		}
	}
