...
LL ppp(LL const base, LL const exponent, LL const mod) {
	if (exponent == 0) {
		return {1};
	}
	auto half = ppp(base, exponent / 2, mod);
	if (exponent % 2 == 0) {
...
int main(int, char const *[]) {
	...
	RF(t, 0, T) {
		...
		RF(i, 0, N) {
			cin >> A[i];
			RF(j, 0, P.size()) {
				h[0][j] = (h[0][j] * R + A[i]) % P[j];
			}
		}
		RF(i, 0, N) {
			cin >> B[i];
			RF(j, 0, P.size()) {
				h[1][j] = (h[1][j] * R + B[i]) % P[j];
			}
		}
		if (N == 2) {
			if (K % 2 == 0) {
		...
		RF(i, 0, tryU) {
			...
			RF(j, 0, P.size()) {
				h[0][j] = (h[0][j] * R + A[i] - (A[i] * RP[j]) % P[j] + P[j]) % P[j];
			}
		}
		cout << (done ? "YES" : "NO") << '\n';
	}
