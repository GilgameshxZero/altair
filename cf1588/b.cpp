...
int main(int argc, char const *argv[]) {
	...
	while (T--) {
		...
		LL left = 3, right = N, ans;
		bool done = false;
		while (left + 1 < right && !done) {
			LL mid = (left + right) / 2;
			cout << "? 1 " << mid << '\n' << flush;
			cin >> ans;
			if (ans == -1) {
			...
			if (c == -1) {
				...
				LL first = c - 1, remain = total - first * (first - 1) / 2,
					 second = calc(remain);
				if (first > 1 && remain > 0 && second == -1) {
					left = mid;
				} else {
					cout << "? 1 " << mid - first << '\n' << flush;
					LL cf;
					...
					if (calc(cf) == first) {
						...
						LL j = mid - first + 1, i = j - first, k = j + second - 1;
						cout << "! " << i << ' ' << j << ' ' << k << '\n' << flush;
						break;
					} else {
						left = mid;
		...
		if (!done) {
			...
			LL i = j - calc(cj), k = j - 1 + calc(total - cj);
			cout << "! " << i << ' ' << j << ' ' << k << '\n' << flush;
		}
	}
