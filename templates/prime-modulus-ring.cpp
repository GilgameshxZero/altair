...
// modulus integer. Template specializations may not use dependent types, so we
// must either use `integral_constant` or `enable_if` to specify the
// this->primeModulus().
...
class PrimeModulusRing : public PrimeModulus {
	...
	// In C++17+, these variables may be declared static inline. To preserve C++11
	// compatibility, we access them via a function instead.
	...
	inline PrimeModulusRing<Integer, PrimeModulus> choose(Integer const K) const {
		if (K < 0 || K > this->value) {
			return {0};
		}
	...
	inline bool operator==(PrimeModulusRing<Integer, PrimeModulus> const other) {
		return this->value == other.value;
	}
};
...
inline PrimeModulusRing<Integer, PrimeModulus> operator+(
	...
	PrimeModulusRing<Integer, PrimeModulus> const right) {
	return PrimeModulusRing<Integer, PrimeModulus>(left) + right;
}
