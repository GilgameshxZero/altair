...
class BoundaryTree : public SegmentTree<PR<LL, LL>, LL> {
	public:
	BoundaryTree(std::size_t const size) : SegmentTree(size) {
		RF(i, 0, this->values.size()) { this->values[i].second = i; }
	}
...
class BoundaryTree2 : public SegmentTree<PR<LL, LL>, LL> {
	public:
	BoundaryTree2(std::size_t const size) : SegmentTree(size) {
		RF(i, 0, this->values.size()) { this->values[i].second = i; }
	}
...
class T3 : public SegmentTree<LL, LL> {
...
int main(int argc, char const *argv[]) {
	...
	RF(i, 0, Q) {
		...
		auto x = X.query(0, N + 2), z = Z.query(0, N + 2);
		LL ans = min(count[0], min(count[1], count[2]));
		if (x.second < z.second) {
			ans = min(
				ans,
				x.first + N - Y.sum(N + 1) + Y.sum(x.second) + z.first + N -
					Y.sum(z.second - 1) + Y.sum(z.second - 1) - Y.sum(x.second));
		}
		ans = min(ans, T.query(0, N + 2) + N);
		cout << ans << '\n';
	}
