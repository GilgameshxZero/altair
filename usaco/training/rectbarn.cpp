...
int main() {
	...
	cin.tie(nullptr);
#ifndef _DEBUG
	std::freopen("rectbarn.in", "r", stdin);
	std::freopen("rectbarn.out", "w", stdout);
#endif

	LL R, C, P;
	cin >> R >> C >> P;
	// TODO: Not sure why the unordered_set template doesn't compile on USACO.
	set<pair<int, int>> XX;
	vector<vector<int>> XU(2, vector<int>(C + 2, 0)), XL{XU},
		XR{XU}, YL{XU}, YR{YL};
	RF(i, 0, P) {
		int j, k;
		cin >> j >> k;
	...
	int Z{0};
	fill(XU[0].begin(), XU[0].end(), 0);
	fill(YL[0].begin(), YL[0].end(), INT_MAX / 3);
	fill(YR[0].begin(), YR[0].end(), INT_MAX / 3);
	RF(i, 1, R + 1) {
		XL[1][0] = XR[1][C + 1] = 0;
		RF(j, 1, C + 1) {
			XU[1][j] = XX.count({i, j}) ? 0 : XU[0][j] + 1;
			XL[1][j] = XX.count({i, j}) ? 0 : XL[1][j - 1] + 1;
		}
		RF(j, C, 0) {
			XR[1][j] = XX.count({i, j}) ? 0 : XR[1][j + 1] + 1;
		}
		RF(j, 1, C + 1) {
			if (XU[1][j] == 0) {
				YL[1][j] = YR[1][j] = INT_MAX / 3;
			} else {
				YL[1][j] = min(XL[1][j], YL[0][j]);
				YR[1][j] = min(XR[1][j], YR[0][j]);
			}
		}
		RF(j, 1, C + 1) {
			Z = max(Z, XU[1][j] * (YL[1][j] + YR[1][j] - 1));
		}
		swap(XU[0], XU[1]);
		swap(XL[0], XL[1]);
		swap(XR[0], XR[1]);
