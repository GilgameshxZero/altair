...
LL const MAX_N = 250;
AR<AR<LL, MAX_N * 2>, MAX_N * 2> C, pre[2];
LL T, N;
...
int main(int argc, char const *argv[]) {
	...
	while (T--) {
		...
		cout << init +
				min(min(
							min(C[0][N], C[0][2 * N - 1]),
							min(C[N - 1][N], C[N - 1][2 * N - 1])),
						min(
							min(C[N][0], C[N][N - 1]),
							min(C[2 * N - 1][0], C[2 * N - 1][N - 1])))
				 << '\n';

		// RF(j, 0, 2 * N) { pre[0][0][j] = C[0][j]; }
		// RF(i, 1, 2 * N) {
		// 	RF(j, 0, 2 * N) { pre[0][i][j] = C[i][j] + pre[0][i - 1][j]; }
		// }
		// RF(i, 0, 2 * N) { pre[1][i][0] = C[i][0]; }
		// RF(i, 0, 2 * N) {
		// 	RF(j, 1, 2 * N) { pre[1][i][j] = C[i][j] + pre[1][i][j - 1]; }
		// }
		...
		// 			min(
		// 				min(
		// 					pre[0][2 * N - 1][j] - pre[0][i][j] + pre[1][i][j],
		// 					pre[0][2 * N - 1][j] - pre[0][i][j] + pre[1][i][2 * N - 1] -
		// 						pre[1][i][j - 1]),
		// 				min(
		// 					pre[0][i][j] + pre[1][i][j - 1],
		// 					pre[0][i][j] + pre[1][i][2 * N - 1] - pre[1][i][j - 1])));
		// 	}
		// }
		// RF(i, N, N * 2) {
		// 	RF(j, 0, N) {
