...
Useful constants: INT_MAX, INT_MIN, UINT_MAX, LLONG_MAX, LLONG_MIN, ULLONG_MAX, _I64_MIN, _I64_MAX, _UI64_MAX.
*/
...
const unsigned int	MAX_CVERT	= 200;
...
int main ()
{
	...
	NetworkEdge temp_edge;
	NetworkVertice net_vert[MAX_CVERT];
	unsigned int cedge, cvert;
	...
	{
		in >> b >> c >> d;
		temp_edge.destination = &net_vert[c - 1];
		temp_edge.edge_cap = d;
		net_vert[b - 1].neighbor.push_back (temp_edge);
	}

	out << MaxFlow (net_vert, 0, cvert - 1, cvert) << "\n";
	out.close ();
