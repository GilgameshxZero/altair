...
int main(int, char const *[]) {
	...
	RF(i, 0, N) { cin >> A[i]; }
	cin >> Q;
	VR<VR<Query>> Q_(N / sqN + 1);
	RF(i, 0, Q) {
		LL L, R, X;
		cin >> L >> R >> X;
		LL bin{L / sqN};
		Q_[bin].push_back({L, R, X, i});
	}
	RF(i, 0, Q_.size()) {
		sort(Q_[i].begin(), Q_[i].end(), [](Query const &X, Query const &Y) {
			return X.R < Y.R;
		});
	}
	LL l, r;
