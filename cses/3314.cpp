...
namespace Rain::Algorithm {
	...
	class DisjointSetUnion {
		...
		inline bool join(std::size_t const i,
			std::size_t const j,
			std::size_t length = 0) {
...
using Rain::Algorithm::DisjointSetUnion;
...
int main() {
	...
	sort(I.begin(), I.end());
	H[0] = H.back() = LLONG_MAX;
	DisjointSetUnion D(N + 2);
	vector<LL> Z(N + 2);
	RF(i, 0, N) {
		if (i > 0 && H[I[i].second] > H[I[i - 1].second]) {
			RF(j, i - 1, -1) {
				if (H[I[j].second] != H[I[i - 1].second]) {
					break;
				}
				if (H[I[j].second - 1] < H[I[j].second]) {
					LL x{max(Z[D.find(I[j].second)], Z[D.find(I[j].second - 1)])};
					D.join(I[j].second, I[j].second - 1);
					Z[D.find(I[j].second)] = x;
				}
				if (H[I[j].second + 1] < H[I[j].second]) {
					LL x{max(Z[D.find(I[j].second)], Z[D.find(I[j].second + 1)])};
					D.join(I[j].second, I[j].second + 1);
		...
		Z[I[i].second] = 1;
		if (H[I[i].second - 1] < H[I[i].second]) {
			Z[I[i].second] = max(Z[I[i].second], 1 + Z[D.find(I[i].second - 1)]);
		}
		if (H[I[i].second + 1] < H[I[i].second]) {
			Z[I[i].second] = max(Z[I[i].second], 1 + Z[D.find(I[i].second + 1)]);
		}
