...
int main() {
	...
	cin >> N >> Q;
	vector<LL> X(N);
	unordered_map<LL, set<LL>, Rain::Random::SplitMixHash<LL>> W;
	SegmentTreeLazy<SegmentTreeLazy<>::PolicyMin<LL>> tree(N);
	LL INF{N};
	RF(i, 0, N) {
		cin >> X[i];
	}
	RF(i, N - 1, -1) {
		set<LL> &j{W[X[i]]};
		auto it{j.insert(i).first};
		it++;
		tree.update(i, i, it == j.end() ? INF : *it);
	}
	RF(i, 0, Q) {
		LL a, b, c;
		cin >> a >> b >> c;
		if (a == 1) {
			b--;
			{
				...
				if (it == j.begin()) {
					...
					auto itn{j.erase(it)};
					tree.update(
						*itp, *itp, (itn == j.end() ? INF : *itn) - tree.query(*itp, *itp));
				}
			}
			...
			{
				set<LL> &j{W[c]};
				auto it{j.insert(b).first};
				tree.update(
					b, b, (next(it) == j.end() ? INF : *next(it)) - tree.query(b, b));
				if (it != j.begin()) {
					it--;
					tree.update(*it, *it, b - tree.query(*it, *it));
				}
			}
		} else {
			b--;
			c--;
			cout << (tree.query(b, c) <= c ? "NO" : "YES") << '\n';
		}
	}
