...
int main() {
	...
	CH(1, 0);
	vector<pair<pair<LL, LL>, LL>> Y(N);
	RF(i, 1, N + 1) {
		Y[i - 1] = {{V[i].OO, V[i].OI}, i};
	}
	sort(Y.begin(), Y.end());
	LL yc{1};
	SegmentTreeLazy<SegmentTreeLazy<>::PolicySum<LL>> tree(N + 1);
	unordered_map<LL, LL, Rain::Random::SplitMixHash<LL>> M;
	vector<LL> Z(N + 1);
	RF(i, 0, N) {
		for (; yc <= Y[i].first.first; yc++) {
			auto it{M.find(V[O[yc]].V)};
			if (it == M.end()) {
				tree.update(1, yc, 1);
				M[V[O[yc]].V] = yc;
			} else {
				tree.update(it->second + 1, yc, 1);
				it->second = yc;
			}
		}
		Z[Y[i].second] = tree.query(Y[i].first.second, Y[i].first.second);
	}
	RF(i, 1, N + 1) {
		cout << Z[i] << ' ';
	}
