...
using MF = Rain::Algorithm::ModulusField<LL, HASH_PRIME>;
...
int main() {
	...
	string S;
	cin >> S;
	vector<LL> P(S.length() + 1), HPP(S.length() + 1);
	P[0] = 0;
	HPP[0] = 1;
	RF(i, 1, S.length() + 1) {
		P[i] = (P[i - 1] * HASH_POWER + S[i - 1] - 'a' + 1) % HASH_PRIME;
		HPP[i] = HPP[i - 1] * HASH_POWER % HASH_PRIME;
	}

	RF(i, 1, S.length() + 1) {
		LL reps(S.length() / i), rem(S.length() % i);
		LL hash{
			P[i] * (HASH_PRIME + 1 - HPP[reps * i]) % HASH_PRIME *
				MF{1 - HPP[i]}.power(HASH_PRIME - 2).value % HASH_PRIME * HPP[rem] +
			P[rem]};
		if (hash % HASH_PRIME == P[S.length()]) {
			cout << i << ' ';
		}
	}
