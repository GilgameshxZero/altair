...
int main() {
	...
	LL p{-1}, c{0};
	vector<LL> I2S;
	unordered_map<LL, LL, Rain::Random::SplitMixHash<LL>> S2I;
	RF(i, 0, T.size()) {
		if (T[i] == p) {
			continue;
		}
	...
	FenwickTree<LL> F(I2S.size());
	RF(i, 0, N) {
		F.modify(S2I[S[i]], 1);
	}
	for (auto &i : X) {
		if (i.first) {
			cout << F.sum(S2I[i.second.second]) - F.sum(S2I[i.second.first] - 1)
					 << '\n';
		} else {
			F.modify(S2I[S[i.second.first - 1]], -1);
			S[i.second.first - 1] = i.second.second;
			F.modify(S2I[i.second.second], 1);
		}
	}
