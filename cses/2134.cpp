...
class Path {
	...
	LL P;
	vector<LL> V;
	unique_ptr<SegmentTreeLazy<SegmentTreeLazy<>::PolicyMax<LL>>> T;
};
...
int main() {
	...
	RF(i, 0, P.size()) {
		P[i].T.reset(
			new SegmentTreeLazy<SegmentTreeLazy<>::PolicyMax<LL>>(P[i].V.size()));
		RF(j, 0, P[i].V.size()) {
			P[i].T->update(j, j, V[P[i].V[j]].V);
		}
	}
	RF(i, 0, Q) {
		LL a, b;
		...
		if (a == 1) {
			...
			LL d{b - V[a].V};
			V[a].V = b;
			P[V[a].P].T->update(V[a].PI, V[a].PI, d);
		} else {
			cin >> a >> b;
			if (V[a].D < V[b].D) {
				swap(a, b);
			}
			LL c{a}, r{V[a].D - V[b].D}, l{V[0].A.size() - 1};
			for (LL l{V[0].A.size() - 1}; l >= 0 && r > 0; l--) {
				if ((1LL << l) <= r) {
					r -= (1LL << l);
					c = V[c].A[l];
				}
			...
			LL d{b};
			for (LL l{V[0].A.size() - 1}; l >= 0 && c != d; l--) {
				if (V[c].A[l] != V[d].A[l]) {
					c = V[c].A[l];
					d = V[d].A[l];
				}
			...
			LL z{LLONG_MIN};
			while (V[a].P != V[c].P) {
				z = max(z, P[V[a].P].T->query(0, V[a].PI));
				a = P[V[a].P].P;
			}
			z = max(z, P[V[a].P].T->query(V[c].PI, V[a].PI));
			while (V[b].P != V[c].P) {
				z = max(z, P[V[b].P].T->query(0, V[b].PI));
				b = P[V[b].P].P;
			}
			z = max(z, P[V[a].P].T->query(V[c].PI, V[b].PI));
			cout << z << ' ';
		}
	}
