...
int main() {
	...
	vector<unordered_multiset<size_t>> E(N);
	RF(i, 0, M) {
		LL A, B;
		cin >> A >> B;
		A--;
	...
	auto C{eulerCycleHeirholzer(E, true)};
	if (C.empty()) {
		cout << "IMPOSSIBLE";
		return 0;
	}
	LL start{0};
	for (; start < C.size(); start++) {
		if (C[start] == N - 1 && C[(start + 1) % C.size()] == 0) {
			start = (start + 1) % C.size();
			break;
		}
	}
	RF(i, 0, C.size()) {
		cout << C[(i + start) % C.size()] + 1 << ' ';
	}
