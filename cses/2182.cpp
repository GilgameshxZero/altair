...
namespace Rain::Algorithm {
	...
	class ModulusRingBase {
		...
		template <
			typename Integer,
			std::enable_if<!isDerivedFromModulusRing<Integer>::value>::type * =
				nullptr>
	...
	class ModulusField : public ModulusRingBase<
		...
		template <
			typename Integer,
			std::enable_if<!std::is_same<TypeThis, Integer>::value>::type * = nullptr>
...
using MR = ModulusRing<LL, 2000000012>;
...
int main() {
	...
	RF(i, 0, N) {
		...
		T *= F[i].second + 1;
		Z0 *= F[i].second + 1;
		Z1 *= (MF{F[i].first}.power(F[i].second + 1) - 1) / (F[i].first - 1);
	}
	RF(i, 0, N) {
		Z2 *= MF{F[i].first}.power(T.value * F[i].second / 2);
	}
	cout << Z0 << ' ' << Z1 << ' ' << Z2;
