...
int main(int, char const *[]) {
	...
	if (K == 0) {
		MR ans{0};
		RF(i, 0, N) { ans = (ans * 10) + S[i] - '0'; }
		cout << ans;
		return 0;
	}
	MR pRow{1}, towerSum{0};
	RF(i, 1, N - 1) { pRow *= i; }
	RF(i, 1, K) { pRow /= i; }
	RF(i, 1, N - K) { pRow /= i; }
	VR<MR> chooseTower, powerTower;
	chooseTower.push_back(1);
	chooseTower.push_back(pRow);
	powerTower.push_back(1);
	powerTower.back() /= 10;
	powerTower.push_back(1);
	towerSum = pRow;
	RF(i, 1, N - 1) {
		...
		chooseTower.push_back(pRow);
		powerTower.push_back(powerTower.back() * 10);
		towerSum += chooseTower.back() * powerTower.back();
	}
	pRow = 0;
	MR ans{0};
	RF(i, 0, N) {
		auto P{N - 1 - i};
		ans += (S[i] - '0') * (towerSum + pRow * 10 * powerTower.back());
		towerSum -= powerTower.back() * chooseTower.back();
		powerTower.pop_back();
		pRow = chooseTower.back();
		chooseTower.pop_back();
		if (N - P == K) {
			pRow = 1;
		} else if (N - P < K) {
			pRow = 0;
