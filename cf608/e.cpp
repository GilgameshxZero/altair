...
int main(int, char const *[]) {
	...
	auto sixy = [&](char dir, LL x, LL y) {
		...
		return PR<LL, LL>{(X[x] == dir) - bx, (Y[y] == dir) - by};
	};
	LL it = 0;
	while (!pq.empty()) {
		it++;
		if ((it & 1023) == 0) {
			if (chrono::high_resolution_clock::now() - tps >= 1500ms) {
				cout << "NO";
				return 0;
			}
		}
		...
		LL zx = z.second, zy = z.first - z.second;
		auto del = sixy('N', zx, zy);
		ci(zx + del.first, zy + del.second);
		del = sixy('E', zx, zy);
		ci(zx + del.first, zy + del.second);
		del = sixy('S', zx, zy);
		ci(zx + del.first, zy + del.second);
