...
int main(int argc, char const *argv[]) {
	...
	while (T--) {
		...
		RF(i, 0, 1 << N) {
			AR<bool, 10> target;
			RF(j, 0, 10) { target[j] = (1 << j) & i; }
			VR<PR<LL, LL>> tt(M);
			RF(j, 0, M) { tt[j].second = j; }
			RF(j, 0, N) {
				RF(k, 0, M) {
					if (S[j][k] == '1') {
						tt[k].first += target[j] ? 1 : -1;
					}
				}
			}
			...
			RF(j, 0, N) {
				LL score = 0;
				RF(k, 0, M) {
					if (S[j][tt[k].second] == '1') {
						score += k + 1;
					}
				}
				surprise += abs(score - X[j]);
			}
			if (surprise > best) {
				best = surprise;
				RF(j, 0, M) { ans[tt[j].second] = j; }
			}
		}
		RF(i, 0, M) { cout << ans[i] + 1 << ' '; }
		cout << '\n';
	}
