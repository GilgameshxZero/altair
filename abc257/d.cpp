...
LL reachable(VR<VR<LL>> &edge, VR<bool> &vis, LL cur) {
	if (vis[cur]) {
		return 0;
	}
	vis[cur] = true;
	...
	RF(i, 0, edge[cur].size()) {
		ans += reachable(edge, vis, edge[cur][i]);
	}
	return ans;
}

bool viable(VR<PR<PR<LL, LL>, LL>> &P, LL S) {
	VR<VR<LL>> edge(P.size());
	RF(i, 0, P.size()) {
		RF(j, 0, P.size()) {
			if (i == j) {
				continue;
			...
			if (
				S * P[i].second >= abs(P[i].first.first - P[j].first.first) +
	...
	RF(i, 0, P.size()) {
		VR<bool> vis(P.size(), false);
		if (reachable(edge, vis, i) == P.size()) {
			return true;
		}
	}
	return false;
