...
int main(int, char const *[]) {
	...
	VR<LL> sd1(N, 1000000000), sdN(N, 1000000000);
	priority_queue<PR<LL, LL>> q;
	q.push({0, 0});
	while (!q.empty()) {
		LL d{-q.top().first}, cur{q.top().second};
		q.pop();
		if (sd1[cur] != 1000000000) {
			continue;
		}
		sd1[cur] = d;
		RF(i, 0, edge[cur].size()) {
			if (sd1[edge[cur][i]] == 1000000000) {
				q.push({-d - 1, edge[cur][i]});
			}
		}
	}
	...
	while (!q.empty()) {
		LL d{-q.top().first}, cur{q.top().second};
		q.pop();
		if (sdN[cur] != 1000000000) {
			continue;
		}
		sdN[cur] = d;
		RF(i, 0, edge[cur].size()) {
			if (sdN[edge[cur][i]] == 1000000000) {
				q.push({-d - 1, edge[cur][i]});
			}
		}
	}

	LL dU1{1000000000}, dUN{1000000000};
	RF(i, 0, bundle.size()) {
		dU1 = min(dU1, sd1[bundle[i]]);
		dUN = min(dUN, sdN[bundle[i]]);
	}

	RF(i, 0, N) {
		LL ans{sd1[N - 1]};
		ans = min(ans, sd1[i] + 1 + dUN);
		ans = min(ans, dU1 + 1 + sdN[i]);
		ans = min(ans, dU1 + 2 + dUN);
