...
def get_notebook_name(
	...
	if is_notebook_kernel_code(global_scope):
		return os.path.basename(
			global_scope.get("__vsc_ipynb_file__"))
	session_name = os.getenv(
		"JPY_SESSION_NAME") or global_scope.get("__session__")
...
def import_global(
	...
	Use this statement to import inside a function, but
	effective as import at the top of the module. Optionally,
	use `value` to set a global variable.
	"""
	if short_name == "":
...
def import_globals_notebook(
		global_scope: typing.Dict[str, str]):
	"""
	Import commonly used globals under standard namespaces.
	Should pass `globals()` as the first argument.

  Typically, call at the top of the notebook. Supports both
