...
def submit(submit_url, cookies, problem, language, files, mainclass='', tag='', ords=[226, 142, 149]):
    """Make a submission.
    ...
    for f in files:
        with open(f, "rb") as sub_file:
            sub_files.append(('sub_file[]',
                            #   (os.path.basename(f),
                              (FILENAME,
                            #    bytes(ords) + sub_file.read()[3:],
                               sub_file.read(),
                            #    "print('alf')",
                               'application/octet-stream')))

    print(data, sub_files)
